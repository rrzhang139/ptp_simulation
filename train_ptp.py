...
def parse_args():
    ...
    parser.add_argument("--exp-name", type=str, default='ptp',
    ...
    parser.add_argument("--past-horizon", type=int, default=8)
    # TODO
    ...
    if args.past_horizon > 0:
        args.obs_horizon  = args.past_horizon + 1
        args.pred_horizon = args.past_horizon + args.pred_horizon # TODO: don't know if I should add the pred or the act horizon
...
class Agent(nn.Module):
    ...
    def compute_loss(self, obs_seq, action_seq):
        ...
        #     sqrt_alphas_cumprod      = self.noise_scheduler.alphas_cumprod[timesteps] ** 0.5
        #     sqrt_one_minus_alphas_cp = (1.0 - self.noise_scheduler.alphas_cumprod[timesteps]) ** 0.5
        #     sqrt_alphas_cumprod      = sqrt_alphas_cumprod[:, None, None]
        #     sqrt_one_minus_alphas_cp = sqrt_one_minus_alphas_cp[:, None, None]
        #     x0_pred = (noisy_action_seq - sqrt_one_minus_alphas_cp * noise_pred) / sqrt_alphas_cumprod
            
        #     loss_past = F.mse_loss(x0_pred, self.past_actions[:, -self.past_horizon:])
                
        #     if self.num_executed >= self.past_horizon:
                
        #         loss += loss_past
            
        #     self.num_executed += self.act_horizon
        #     self.past_actions = torch.cat([self.past_actions, action_seq[:, :self.act_horizon]], axis=1)
